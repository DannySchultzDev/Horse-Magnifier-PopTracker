...
function apply_slot_data(slot_data)
	...
	if slot_data['perfect_accuracy'] then
		local perfect_accuracy = Tracker:FindObjectForCode("Perfect Accuracy")
		perfect_accuracy.Active = (slot_data['perfect_accuracy'] ~= 0)
	end
	if slot_data['leniency'] then
		local leniency = Tracker:FindObjectForCode("Leniency")
		leniency.AcquiredCount = (slot_data['leniency'])
	end
	if slot_data['horse_amount'] and slot_data['goal_requirement'] then
		local goal_requirement = Tracker:FindObjectForCode("Goal Requirement")
		goal_requirement.AcquiredCount = (slot_data['horse_amount'] * slot_data['goal_requirement'] / 100)
	end
	if slot_data['deathlink'] then
		local deathlink = Tracker:FindObjectForCode("Deathlink")
		deathlink.CurrentStage = (slot_data['deathlink'])
	end
end
